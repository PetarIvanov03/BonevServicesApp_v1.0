...
{
    public partial class CarMainCategories : Form
    {
        private bool openAnotherForm = false;
        private Car _foundCar;
        public CarMainCategories(Car foundCar)
        {
            InitializeComponent();

            this._foundCar = foundCar;

            this.Text = $"Bonev Services - {foundCar.Name} - Категории";
            this.label1.Text = $"{_foundCar.Name} - Категории";
            ...
            foreach (var item in Categories.autoChasti)
            {
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            if (sender is Button clickedButton && clickedButton.Tag is KeyValuePair<string, List<string>> mainCategory)
            {
                CarSubCategories f = new CarSubCategories(_foundCar, mainCategory);
                f.Show();
            }
        }
